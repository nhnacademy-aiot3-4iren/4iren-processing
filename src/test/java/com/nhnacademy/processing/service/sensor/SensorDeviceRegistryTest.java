...
class SensorDeviceRegistryTest {
    ...
    void ensureRegistered_DeviceCaching() {
        // given
        DeviceIdentity device = new DeviceIdentity("app1", "app", "prof1", "dev1", DEV_EUI, ROOM_ID, "point");
        ...
        // when 1차 수신
        registry.ensureRegistered(message, BROKER_ID, ROOM_ID);

        // when 2차 수신 (동일 devEui)
        registry.ensureRegistered(message, BROKER_ID, ROOM_ID);

        // then: registerDeviceIfAbsent는 1번만 실행되어야 함
        verify(sensorDeviceService, times(1)).registerDeviceIfAbsent(message, DEV_EUI, BROKER_ID, ROOM_ID);
    }
